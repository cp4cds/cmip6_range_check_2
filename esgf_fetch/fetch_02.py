...
## for some reason "ex1" works on http, but ex_tas needs https
...
tmpl_tas = "https://esgf-index1.ceda.ac.uk/esg-search/search/?offset=0&limit=500&type=Dataset&replica=false&latest=true&project%%21=input4mips&activity_id=CMIP&%(filters)s&facets=mip_era%%2Cactivity_id%%2Cmodel_cohort%%2Cproduct%%2Csource_id%%2Cinstitution_id%%2Csource_type%%2Cnominal_resolution%%2Cexperiment_id%%2Csub_experiment_id%%2Cvariant_label%%2Cgrid_label%%2Ctable_id%%2Cfrequency%%2Crealm%%2Cvariable_id%%2Ccf_standard_name%%2Cdata_node&format=application%%2Fsolr%%2Bjson"
...
class ESGFQS(object):
    def __init__(self,facets = ['variable_id','table_id','mip_era','table_id','experiment_id','source_id','activity_id','institution_id','sub_experiment_id','variant_label','grid_label','frequency','realm']):
        self.facets = facets
        self.qtemplate = "https://esgf-index1.ceda.ac.uk/esg-search/search/?%(config)s&%(filters)s&%(facet_list)s&format=application%%2Fsolr%%2Bjson"
        self.config = "offset=0&limit=500&type=Dataset&replica=false&latest=true"
        self.facet_list = "facets=mip_era%2Cactivity_id%2Cmodel_cohort%2Cproduct%2Csource_id%2Cinstitution_id%2Csource_type%2Cnominal_resolution%2Cexperiment_id%2Csub_experiment_id%2Cvariant_label%2Cgrid_label%2Ctable_id%2Cfrequency%2Crealm%2Cvariable_id%2Ccf_standard_name%2Cdata_node"

    def __call__(self, **kwargs ):
        bad = [k for k in kwargs if k not in self.facets]
        assert len(bad) == 0, 'facet(s) %s not supported' % bad
        self.facet_values = kwargs
        self._query_filter()
        self.qstring = self.qtemplate % self.__dict__
        return self.qstring

    def _query_filter(self):
        self.filters = '&'.join( ['%s=%s' % (k,v) for k,v in self.facet_values.items()] )
        return self.filters
...
class ESGF_Query(ESGFQS):

  def submit(self,query=None,resp=None, filters=None):

    if filters != None:
        if type (filters) == type(''):
            self.query=query % dict( filters=filters )
        else:
           self.query=query % dict( filters='&'.join( ['%s=%s' % (k,v) for k,v in filters.items()] ) )
    elif query == None:
        self.query = self.qstring
    else:
        self.query = query

    with urllib.request.urlopen(self.query) as response:
