import glob, os, stat, re, string, filecmp, collections, ftplib, netrc
...
class FTPsession(object):
  def __init__(self):
    self.ftp = ftplib.FTP(host='89.187.86.6')
    self.ftp.connect()
    au = Auth()
    this = au.hosts['89.187.86.6']
    self.ftp.login(user=this[0],passwd=this[2])
  ...
  def type(self,path):
    try:
      ...
      return -1
    c = re.search('ype=(icdir|pdir|dir|file);', return_str)
    if c == None:
      return -2
    return c.group(1)
...
### these need testing ####
  def mkdir(self,dirname):
    """Make a sub-directory in current directory"""
    head, tail = os.path.split( dirname.strip('/') )
    if head != '':
      self.cd(head)
      return self.ftp.mkd( tail )
    return self.ftp.mkd( dirname )

  def cd(self,dirname):
    """Change current directory"""
    return self.ftp.cwd( dirname )


class Auth(object):
  def __init__(self):
    rcmode = oct( os.stat( '%s/.netrc' % HOME ).st_mode )[-2:]
    assert rcmode == '00', 'File permissions on .netrc are not safe ... please fix'
    cc = collections.defaultdict( dict )
    self.rc = netrc.netrc('%s/.netrc' % HOME )
    self.hosts = self.rc.hosts
...
class publish:
  ...
  def __init__(self, root1, root2, levNoChange=0, levChange=1, queryChange=True):
    assert os.path.isdir( root1 ), 'Directory %r not found' % r
    if root1[-1] != '/':
      root1 += '/'
    if root2[-1] != '/':
      root2 += '/'
...
## code to make target directory removed here .. possibility of failing in this case.
...
if __name__ == "__main__":
  au = Auth() 
  assert '89.187.86.6' in au.hosts, 'Site not found in netrc: %s' % au.hosts.keys()
