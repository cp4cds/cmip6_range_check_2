...
c   gfortran -o read_nc.x -I/usr/include read_nc.f -L/usr/lib -lnetcdf -lnetcdff
c /usr/include must contain the netcdf.mod file -- the default installation
c of netcdf sometimes omits this file.
c The order of -I , -L etx is important
c To run: ./read_nc.x
c ==============
      program main
      character (len=60) :: file

      file = 'cru_ts4.01.1901.2016.pre.dat.nc'
...
c ======================================
     ...
     :            k, k2, ndims,  i, l1,l2,l3, k_un, mvs
     ...
     :          k,name(1:20),ndims, dimids(1:ndims), xtype, natts
        if ( name(1:4).eq.'pre' ) then
          l1 = lendims(dimids(1))
          l2 = lendims(dimids(2))
          l3 = lendims(dimids(3))
          ...
          if (natts.gt.0) then
            ...
            status = nf90_get_att(ncid, k, 'missing_value', rval )
            if (status.ne.0) then
                mvs = -1
            else
                mvs = 0
                mval = rval
                write( 6,*) '==== missing value ====',mval
            endif
          endif
        endif
      enddo
      ...
      if (mvs.eq.-1) then
        ...
        write(6,*) maxval(data, data.ne.mval),
     :             minval(data, data.ne.mval) 
      endif
